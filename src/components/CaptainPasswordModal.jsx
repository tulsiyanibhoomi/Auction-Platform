...
export default function CaptainPasswordModal({ isOpen, onSuccess, onClose, teamId, teamName }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/verify-captain', {
        method: 'POST',
    ...
    <div className="admin-auth-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="admin-auth-modal__icon-wrapper">
          <div className="admin-auth-modal__icon">👨‍✈️</div>
          <div className="admin-auth-modal__icon-ring" />
        </div>

        <h2 className="admin-auth-modal__title">Captain Access</h2>
        <p className="admin-auth-modal__subtitle">
          Enter the captain password for <strong>{teamName || 'your team'}</strong>
        </p>
        ...
        <form onSubmit={handleSubmit} className="admin-auth-modal__form">
          ...
          <button
            type="submit"
            className="btn btn-blue btn-lg admin-auth-modal__submit"
            disabled={loading}
            id="captain-auth-submit-btn"
          >
            {loading ? (
